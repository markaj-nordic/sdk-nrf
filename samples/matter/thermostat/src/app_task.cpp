...
{
	AppEvent event(AppEventType::TemperatureButton);

	if (TEMPERATURE_BUTTON_MASK & hasChanged) {
		event.TemperatureButtonEvent.Action = (TEMPERATURE_BUTTON_MASK & state) ?
							      TemperatureButtonAction::Pushed :
							      TemperatureButtonAction::Released;
		TaskExecutor::PostTask([event] { ThermostatHandler(event); });
	}
}

void AppTask::ThermostatHandler(const AppEvent &event)
{
	if (event.TemperatureButtonEvent.Action == TemperatureButtonAction::Pushed) {
